...
ngOnInit() {
  if(!navigator.geolocation){
    console.log('locayion is not supported');
  }
navigator.geolocation.getCurrentPosition((position) => {
  ...
  L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=pk.eyJ1IjoidGVyZXphMDciLCJhIjoiY2t4aXlkcTB2NHdibDJvbzFycHNqZjAyYSJ9.irVCUJrRQDaMxMGHnq1BCg', {
    ...
    let marker1 = L.marker([coords.latitude+0.0005, coords.longitude+0.0007]).addTo(map);
    marker.bindPopup('<b>FREE PARKING</b>').openPopup();
